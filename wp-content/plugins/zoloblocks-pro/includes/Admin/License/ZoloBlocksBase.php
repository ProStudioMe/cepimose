...
namespace ZoloPro\Admin\License;

if (!class_exists("ZoloBlocksBase")) {
	class ZoloBlocksBase {
		public $key = "389DEC7D26DC6051";
		private $product_id = "7";
		private $product_base = "zoloblocks-pro";
		...
		function __construct($plugin_base_file = '') {
			...
			if ($this->has_check_update) {
				if (function_exists("add_action")) {
					add_action('admin_post_zoloblocks_fupc', function () {
					...
					if ($this->is_theme) {
						...
						add_filter('plugin_row_meta', function ($links, $plugin_file) {
							if (plugin_basename($this->plugin_file) == $plugin_file) {
								$links[] = " <a class='edit coption' href='" . esc_url(admin_url('admin-post.php') . '?action=zoloblocks_fupc') . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2);
		...
		public function theme_force_update() {
			...
			echo wp_kses_post('<h1>' . esc_html__("Update Checking..", "zoloblocks") . '</h1>');
		...
		function init_action_handler() {
			...
			if (isset($_GET['action']) && $_GET['action'] == $handler) { // phpcs:ignore
